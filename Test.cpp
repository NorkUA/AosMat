...
int main() {
    ...
    if (!f)
        ...
        f << "Save info about out stacks \nAfter first step  in R1\n"<<for_cout(R1)<< "\nAfter second step in R2\n" << for_cout(R2) << "\nAfter third step in R3\n" << for_cout(R3)
            << "\nAfter fourth step in R4\n" << for_cout(R4) << "\nAfter fifth step in R5\n" << for_cout(R5) << "\nAfter sixth step in R6\n" << for_cout(R6)
            << "\nAfter seventh step in R7\n" << for_cout(R7) << "\nAfter eighth step in R8\n" << for_cout(R8);
        cout << "Info in txt file \n";
        f.close();
        string word ;
        f.open("Out.txt");
        while (f >> word) {
